...
#            for d in self.include_dirs: # KGEN deletion
    ...
    def next(self, ignore_comments = False):
        ...
        try:
            ...
            #logger.critical(red_text('STOPPED READING')) # KGEN  deletion
            logger.critical('STOPPED READING') # KGEN addition
    ...
    def format_message(self, kind, message, startlineno, endlineno,
        ...
        for i in range(startlineno,min(endlineno+back_index,len(self.source_lines))+1):
            ...
            if i==endlineno:
                ...
                #r.append('%s%s%s <== %s' % (l0,yellow_text(l1),l2,red_text(message))) # KGEN deletion
                r.append('%s%s%s <== %s' % (l0,l1,l2,message)) # KGEN addition
            else:
